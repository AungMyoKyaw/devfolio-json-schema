...
console.log(validation1.success ? '✅ PASSED' : '❌ FAILED', '- Comprehensive sample validation');

// Test 2: Safe validation
console.log('\nTest 2: Safe validation test...');
const validation2 = safeValidateDevFolio(sampleDevFolio);
console.log(validation2.success ? '✅ PASSED' : '❌ FAILED', '- Safe validation');
...
console.log(validation3.success ? '✅ PASSED' : '❌ FAILED', '- Minimal object validation');
...
console.log(!validation4.success ? '✅ PASSED' : '❌ FAILED', '- Invalid data rejection');
...
console.log(emptyValidation.success ? '✅ PASSED' : '❌ FAILED', '- Empty object validation');

// Test 8: Schema URL test
console.log('\nTest 8: Schema URL test...');
const hasCorrectSchema = sampleDevFolio.$schema === 'https://github.com/AungMyoKyaw/devfolio-json-schema/v1.0.0';
console.log(hasCorrectSchema ? '✅ PASSED' : '❌ FAILED', '- Schema URL validation');
...
console.log(`Validated 1000 objects in ${endTime - startTime}ms (${((endTime - startTime) / 1000).toFixed(2)}ms per validation)`);
